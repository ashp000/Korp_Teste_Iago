...
6. **Detalhamento técnico** (falar sobre, com base no código):
   - **Ciclos de vida Angular usados:** `ngOnInit` (carga inicial de dados em todas as telas), `ngAfterViewInit` (associação do `MatSort`/`MatPaginator` à tabela de produtos, que só existem após a view renderizar).
